...
def checkpoint_migrate(request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _validate_path_field("source_path", request.get("source_path"), allowed_root=allowed_root, require_exists=True)
    _validate_path_field("target_path", request.get("target_path"), allowed_root=allowed_root, require_exists=False)
    return {"status": "OK"}
