...
- Canonical CBOR is defined by RFC 8949 deterministic encoding rules and `specs/layers/L1-foundation/Canonical-CBOR-Profile.md`.
...
- `Glyphser.Error.Emit` (defined in `specs/layers/L1-foundation/Error-Codes.md`)
...
- `CONTRACT_VIOLATION` emission semantics are governed by `specs/layers/L1-foundation/Error-Codes.md`.
...
- `driver_runtime_fingerprint_hash`:
  - input: canonical CBOR map for `DriverRuntimeFingerprint` from `specs/layers/L1-foundation/Determinism-Profiles.md` section II.H.
  - if dependency document is unavailable or invalid, abort with `CONTRACT_VIOLATION`.
- `determinism_profile_hash`:
  - input: selected profile commitment from `specs/layers/L1-foundation/Determinism-Profiles.md` section II.G.
  - if dependency document is unavailable or invalid, abort with `CONTRACT_VIOLATION`.
...
- `Glyphser.Environment.ValidateCompatibility`
- `Glyphser.Error.Emit`
- External operator reference note: `Glyphser.Error.Emit` is defined in `specs/layers/L1-foundation/Error-Codes.md`.
