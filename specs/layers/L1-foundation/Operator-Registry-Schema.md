...
- Hash/token paths use `SHA-256(CBOR_CANONICAL(...))`.
 - `CBOR_CANONICAL` is normative as defined in `specs/layers/L1-foundation/Canonical-CBOR-Profile.md`.
### 0.C Numeric Policy
...
- These `code_id` values MUST map to authoritative error catalog entries in `specs/layers/L1-foundation/Error-Codes.md`.
...
- `digest_catalog_blob` (canonical CBOR; schema per `specs/layers/L1-foundation/Digest-Catalog.md`)
- `error_codes_blob` (canonical CBOR; schema per `specs/layers/L1-foundation/Error-Codes.md`)
- `capability_catalog_blob` (canonical CBOR; schema per `specs/layers/L2-specs/Security-Compliance-Profile.md`)
...
- Disambiguation:
  ...
  - otherwise treat as catalog label and resolve using the provided `digest_catalog_blob` (whose schema is defined in `specs/layers/L1-foundation/Digest-Catalog.md`).
- Resolution output:
...
- Required fields per operator record:
  ...
  - `allowed_error_codes:array<error_code_id>` where `error_code_id:string` and value MUST exist in the provided `error_codes_blob` (whose schema is defined in `specs/layers/L1-foundation/Error-Codes.md`) (sorted, unique by bytewise UTF-8 code-id order)
...
- Capability validation rule:
  - each `required_capabilities` entry MUST exist in the provided `capability_catalog_blob` (whose schema is defined in `specs/layers/L2-specs/Security-Compliance-Profile.md`).
...
**Purity class:** PURE  
**Determinism:** deterministic  
**Definition:** computes `operator_registry_root_hash = SHA-256(CBOR_CANONICAL(["operator_registry", [registry_schema_version, operator_records]]))` using canonical serialization profile `CanonicalSerialization` from `specs/layers/L1-foundation/Canonical-CBOR-Profile.md`, where `operator_records` are ordered by `(operator_id, version_num)` and `version_num = parse_uint(version[1:])`.
