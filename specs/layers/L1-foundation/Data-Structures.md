...
- Operator-level enums (`purity_class`, `side_effect`) are authoritative in `specs/layers/L1-foundation/Operator-Registry-Schema.md` and `specs/layers/L1-foundation/API-Interfaces.md` and are intentionally not redefined here.
...
- Canonicalization rules:
  - map keys ordered by `(len(CBOR_ENCODE(key)), CBOR_ENCODE(key))` as defined in `specs/layers/L1-foundation/Canonical-CBOR-Profile.md`,
...
- Generation rule is defined in `specs/layers/L2-specs/Replay-Determinism.md`; this document does not redefine token derivation.
...
- E0 required.
- `E0` means byte-identical canonical bytes and identical hash outputs for identical logical structure declarations (aligned with `specs/layers/L2-specs/Replay-Determinism.md`).
...
- `specs/layers/L2-specs/Replay-Determinism.md`
