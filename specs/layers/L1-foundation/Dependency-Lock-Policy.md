...
- Canonical serialization for all typed objects in this contract is canonical CBOR per `specs/layers/L1-foundation/Canonical-CBOR-Profile.md` and RFC 8949 deterministic encoding rules.
...
- `runtime_env_hash` is the canonical environment commitment (`env_manifest_hash`) from `specs/layers/L1-foundation/Environment-Manifest.md`.
- Error payload structure and canonical codes are defined in `specs/layers/L1-foundation/Error-Codes.md`.
...
External operator reference: `Glyphser.Error.Emit` is defined normatively in `specs/layers/L1-foundation/Error-Codes.md`.
