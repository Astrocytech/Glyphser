...
- `signature_digest` rule is defined normatively in `specs/layers/L1-foundation/Operator-Registry-Schema.md` (`sig` preimage over resolved digest values); backend adapter manifests must match exactly.
