...
| `wal_corruption_truncated_tail` | deterministic rollback or finalize path | `specs/layers/L2-specs/Run-Commit-WAL.md` | same as above + wal_recovery_hash | regulated requires proof-pack pass |
| `wal_corruption_checksum_mismatch` | chain integrity detection and deterministic abort | `specs/layers/L2-specs/Run-Commit-WAL.md` | same as above + wal_chain_validation_hash | regulated requires proof-pack pass |
