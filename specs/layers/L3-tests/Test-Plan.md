...
- Fail build if any referenced code is absent from `specs/layers/L1-foundation/Error-Codes.md`.
...
- `tooling/quality_gates/spec_lint.py` is a mandatory pre-test CI gate and must run before unit/integration/replay/performance suites.
- `spec_lint` must verify at minimum:
  ...
  - error-code closure (all referenced codes must exist in `specs/layers/L1-foundation/Error-Codes.md` with valid severity enum),
