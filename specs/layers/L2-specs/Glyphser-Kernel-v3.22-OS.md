...
- `env_manifest_hash` is defined normatively in `specs/layers/L1-foundation/Environment-Manifest.md` (`runtime_env_hash` alias allowed in checkpoint contracts).
...
- `env_manifest_hash` includes `daemon_concurrency_max=16` and all required runtime fields from `specs/layers/L1-foundation/Environment-Manifest.md`.
- `spec_version: string`
- `tenant_id: string`
- `seed: uint64`
...
- Certificate field-set is authoritative in `specs/layers/L2-specs/Execution-Certificate.md` only.
- Kernel and adapters MUST NOT add implicit certificate fields outside `specs/layers/L2-specs/Execution-Certificate.md` `signed_payload` and allowed `unsigned_metadata`.
...
- Core execution specs: `specs/layers/L2-specs/Data-NextBatch.md`, `specs/layers/L2-specs/ModelIR-Executor.md`, `specs/layers/L2-specs/TMMU-Allocation.md`, `specs/layers/L2-specs/DifferentialPrivacy-Apply.md`.
- Interface and type contracts: `specs/layers/L1-foundation/API-Interfaces.md`, `specs/layers/L1-foundation/Data-Structures.md`, `specs/layers/L2-specs/Config-Schema.md`.
- Reliability and observability contracts: `specs/layers/L1-foundation/Error-Codes.md`, `specs/layers/L2-specs/Trace-Sidecar.md`, `specs/layers/L2-specs/Checkpoint-Schema.md`, `specs/layers/L2-specs/Replay-Determinism.md`.
- Delivery and compliance contracts: `docs/layer4-implementation/Backend-Adapter-Guide.md`, `specs/layers/L2-specs/Security-Compliance-Profile.md`, `specs/layers/L1-foundation/Dependency-Lock-Policy.md`, `specs/layers/L2-specs/Deployment-Runbook.md`.
...
- Lifecycle and governance extensions: `specs/layers/L2-specs/Experiment-Tracking.md`, `specs/layers/L2-specs/Model-Registry.md`, `specs/layers/L2-specs/Monitoring-Policy.md`, `specs/layers/L2-specs/Evaluation-Harness.md`, `specs/layers/L2-specs/Data-Lineage.md`, `specs/layers/L2-specs/Pipeline-Orchestrator.md`, `specs/layers/L2-specs/Execution-Certificate.md`.
...
- New contract subdocuments:
  - `specs/layers/L1-foundation/Operator-Registry-Schema.md` (authoritative registry schema),
  - `specs/layers/L1-foundation/Digest-Catalog.md` (digest label/value authority),
  - `specs/layers/L1-foundation/Canonical-CBOR-Profile.md` (single commitment serialization profile),
  - `specs/layers/L2-specs/Run-Commit-WAL.md` (atomic commit journal + recovery),
  - `AuthZ-Capability-Matrix.md` (operator capability policy),
  - `specs/layers/L1-foundation/Determinism-Profiles.md` (BITWISE/TOLERANCE runtime profiles).
- Wiring invariant: all operator names referenced across documents must be fully qualified and versioned. Shared operators may be imported by reference from dedicated contract documents (for example `Glyphser.Error.Emit` in `specs/layers/L1-foundation/Error-Codes.md`).
...
External operator reference: `Glyphser.Commit.WALAppend` and `Glyphser.Commit.FinalizeRunCommit` are defined normatively in `specs/layers/L2-specs/Run-Commit-WAL.md` and imported by reference.
...
**Dependencies:** `specs/layers/L2-specs/TMMU-Allocation.md`.
...
**Purity class:** IO  
**Determinism:** deterministic  
**Definition:** emits canonical `ExecutionCertificate` object exactly as defined in `specs/layers/L2-specs/Execution-Certificate.md`. Legacy `Glyphser.IO.WriteTrainingCertificate` is deprecated and must be implemented as a thin alias wrapper to this operator.
...
- resource_ledger <- update deterministic per-step resource counters `{flops, bytes_allocated, peak_bytes, gpu_time_ns, cpu_time_ns}` by summing declared per-operator contributions for operators executed in this step (operator resource contributions are defined in operator contract metadata; this version assumes fixed per-operator costs).
  # quota policy schema/semantics are governed by `specs/layers/L2-specs/Pipeline-Orchestrator.md` (`QuotaPolicy`)
