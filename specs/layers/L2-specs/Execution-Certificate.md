...
- `signed_payload` required fields:
  ...
  - `policy_gate_hash:bytes32`
    - semantics: hash of the canonical policy-gate evaluation transcript for the certified run context, as defined in `specs/layers/L2-specs/Monitoring-Policy.md` (section II.G).
  ...
  - `dp_epsilon?:float64`
  - `dp_delta?:float64`
  - `dp_accountant_state_hash?:bytes32` (if DP enabled; equals `SHA-256(CBOR_CANONICAL(accountant_state_t))` from `specs/layers/L2-specs/DifferentialPrivacy-Apply.md`)
...
- Trust/revocation commitment definitions:
  ...
  - Normative source definitions are aligned with `specs/layers/L2-specs/Security-Compliance-Profile.md`.
...
External operator reference: `Glyphser.Error.Emit` is defined in `specs/layers/L1-foundation/Error-Codes.md`.
