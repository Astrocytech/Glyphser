...
- Canonical serialization: `CBOR_CANONICAL` as defined in `specs/layers/L1-foundation/Canonical-CBOR-Profile.md`.
...
- Required cross-doc fields:
  ...
  - `environment.env_manifest_hash:bytes32` (as defined in `specs/layers/L1-foundation/Environment-Manifest.md`; alias `runtime_env_hash` must resolve identically)

### II.F.2 Kernel Manifest Alignment (Normative)
- To preserve cross-file consistency with `specs/layers/L2-specs/Glyphser-Kernel-v3.22-OS.md` section `0.Q`, the following top-level manifest fields are recognized by this schema (required/optional as noted):
