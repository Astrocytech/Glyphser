...
- Gate metric definitions (normative):
  ...
  - `replay_determinism_failures` = count of E0 replay-check failures over the same fixed window.
    - failure criterion is a divergence reported by `Glyphser.Replay.CompareTrace` under the applicable comparison profile from `specs/layers/L2-specs/Replay-Determinism.md`.
