...
from tooling.codegen.input_hash_manifest import main as input_hash_manifest_main
...
def _class_name_from_schema(schema: Dict[str, Any], fallback: str) -> str:
    ...
    class_name = "".join(w[:1].upper() + w[1:] for w in words)
    return _sanitize_identifier(class_name)
...
def _render_operator_stubs(records: Iterable[Dict[str, Any]]) -> str:
    ...
    for record in records:
        ...
        if operator_id in implemented:
            ...
            blocks.append(
                '            "error": emit_error('
                '"CONTRACT_VIOLATION", "invalid request", operator_id="%s"'
                ")" % operator_id
            )
            blocks.append("        }")
            ...
            blocks.append('            "t": _placeholder_value(request),')
            blocks.append('            "node_id": _placeholder_value(request),')
            blocks.append('            "instr": _placeholder_value(request),')
            blocks.append('            "backend_binary_hash": _placeholder_value(request),')
            blocks.append('            "failure_operator": _placeholder_value(request),')
            blocks.append('            "driver_runtime_fingerprint_hash": _placeholder_value(request),')
            blocks.append('            "replay_token": _placeholder_value(request),')
            blocks.append('            "dataset_key": _placeholder_value(request),')
            blocks.append('            "global_position": _placeholder_value(request),')
            blocks.append('            "cardinality": _placeholder_value(request),')
            blocks.append('            "shape_in": _placeholder_value(request),')
            blocks.append('            "shape_expected": _placeholder_value(request),')
            blocks.append('            "size": _placeholder_value(request),')
            blocks.append('            "capacity": _placeholder_value(request),')
            blocks.append('            "arena": _placeholder_value(request),')
            blocks.append('            "logical_slot": _placeholder_value(request),')
            blocks.append('            "offset": _placeholder_value(request),')
            blocks.append('            "peak_required_bytes": _placeholder_value(request),')
            blocks.append('            "state_hash": _placeholder_value(request),')
            blocks.append('            "registered_hash": _placeholder_value(request),')
            blocks.append('            "stage_id": _placeholder_value(request),')
            blocks.append('            "ir_hash": _placeholder_value(request),')
            blocks.append('            "grad_hash": _placeholder_value(request),')
            blocks.append('            "rng_offset_before": _placeholder_value(request),')
            blocks.append('            "rng_offset_after": _placeholder_value(request),')
            blocks.append('            "required": _placeholder_value(request),')
            blocks.append('            "accountant": _placeholder_value(request),')
            blocks.append('            "accountant_type": _placeholder_value(request),')
            blocks.append('            "sigma_map_hash": _placeholder_value(request),')
            blocks.append('            "clipping_strategy": _placeholder_value(request),')
            blocks.append('            "cumulative_epsilon": _placeholder_value(request),')
            blocks.append('            "target_epsilon": _placeholder_value(request),')
            blocks.append("        }")
            ...
            if operator_id == "Glyphser.IO.SaveCheckpoint":
                ...
                blocks.append(
                    '                "error": emit_error('
                    '"INPUT_MISSING", "missing path", operator_id="%s"'
                    ")" % operator_id
                )
                blocks.append("            }")
            ...
            blocks.append(
                ...
                "t=_placeholder_value(request), node_id=_placeholder_value(request), "
                "instr=_placeholder_value(request), "
                "backend_binary_hash=_placeholder_value(request))}" % operator_id
            )
        blocks.append("")
    return "\n".join(blocks).strip() + "\n"
