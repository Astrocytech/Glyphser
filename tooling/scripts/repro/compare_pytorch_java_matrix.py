...
JAVA_SRC = ROOT / "tooling" / "scripts" / "repro" / "java_bridge" / "HelloCoreJavaRunner.java"
JAVA_DIR = JAVA_SRC.parent
JAVA_CLASS = "HelloCoreJavaRunner"
...
def _run_java_dense(inputs: list[float], weights: list[float], bias: float) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            # Build fingerprint-like object compatible with comparisons.
            "execution_fp": hashlib.sha256(
...
def _classify_pair(
    ...
    if "error" in b_res:
        return {
            ...
            "status": "BLOCKED",
            "classification": "E2",
            "reason": f"java_cpu failed: {b_res['error']['code_id']}",
            "a_result": a_res,
            "b_result": b_res,
        }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Milestone 8 PyTorch+Java cross matrix.")
    parser.add_argument("--abs-tol", type=float, default=1e-6)
    parser.add_argument("--rel-tol", type=float, default=1e-6)
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "evidence" / "repro" / "milestone-8-pytorch-java"),
    ...
    java_res = _run_java_dense(inputs, weights, bias)
    pair_results: list[dict[str, Any]] = []
    for driver_id, _ in PAIRS:
        py_res = _run_pytorch(driver_id, model_ir, inputs)
        pair_results.append(_classify_pair(driver_id, py_res, java_res, args.abs_tol, args.rel_tol))

    statuses = [r["status"] for r in pair_results]
    if any(s == "FAIL" for s in statuses):
        overall_status = "FAIL"
        overall_class = "E2"
        overall_reason = "At least one PyTorch↔Java pair failed tolerance."
    elif any(s == "BLOCKED" for s in statuses):
        overall_status = "BLOCKED"
        overall_class = "E2"
        overall_reason = "At least one PyTorch↔Java pair is blocked by runtime visibility."
    else:
        classes = [r["classification"] for r in pair_results]
        overall_status = "PASS"
        overall_class = "E0" if all(c == "E0" for c in classes) else "E1"
        overall_reason = "All PyTorch↔Java pairs pass."

    report = {
        "milestone": 8,
        "profile": "pytorch_java_cross_matrix",
    ...
    summary_lines = [
        "# Milestone 8: PyTorch + Java Cross-Combination Reproducibility",
        "",
        ...
        "",
        "## Cross Pairs",
        "- pytorch_cpu vs java_cpu",
        "- pytorch_gpu vs java_cpu",
        "",
    ]
