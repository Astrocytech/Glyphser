...
def _tf_meta() -> dict[str, Any]:
    try:
        import tensorflow as tf
    except Exception as exc:  # pragma: no cover
        return {"tensorflow_present": False, "error": str(exc)}

    gpus = tf.config.list_physical_devices("GPU")
    return {
        "tensorflow_present": True,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare Keras CPU and GPU reproducibility for Glyphser.")
    parser.add_argument("--abs-tol", type=float, default=1e-6)
    parser.add_argument("--rel-tol", type=float, default=1e-6)
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "evidence" / "repro" / "milestone-6-keras-cpu-gpu"),
    ...
    cpu_result = _run_profile("keras_cpu", model_ir, inputs)
    gpu_result = _run_profile("keras_gpu", model_ir, inputs)
    ...
    status = "PASS"
    classification = "E0"
    reason = "Keras CPU and GPU outputs/execution fingerprints match exactly."

    if "error" in cpu_result:
        status = "FAIL"
        classification = "E2"
        reason = f"Keras CPU profile failed: {cpu_result['error']['code_id']}"
    elif "error" in gpu_result:
        status = "BLOCKED"
        classification = "E2"
        reason = f"Keras GPU profile unavailable/failing: {gpu_result['error']['code_id']}"
    else:
        ...
        if same_outputs and same_fp:
            ...
            status = "PASS"
            classification = "E1"
            reason = "Keras CPU/GPU outputs within configured tolerance."
        else:
            status = "FAIL"
            classification = "E2"
            reason = "Keras CPU/GPU outputs diverge beyond tolerance."

    report = {
        "milestone": 6,
        "profile": "keras_cpu_gpu",
        "status": status,
    ...
    summary_lines = [
        "# Milestone 6: Keras CPU + GPU Reproducibility",
        "",
        f"Status: {status}",
