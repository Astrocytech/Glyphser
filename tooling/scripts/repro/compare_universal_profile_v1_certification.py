...
REQUIRED_MILESTONES = {
    9: "milestone-9-operator-expansion-v2",
    10: "milestone-10-model-class-expansion",
    11: "milestone-11-dataset-scale-expansion",
    12: "milestone-12-multi-host-multi-os",
    13: "milestone-13-java-runtime-hardening",
    14: "milestone-14-additional-language-bridges",
    15: "milestone-15-service-reproducibility",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Milestone 16 universal profile v1 certification aggregator.")
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "evidence" / "repro" / "milestone-16-universal-profile-v1"),
    )
    ...
    if failed:
        ...
        overall_reason = "All prerequisite milestones satisfied for Universal Profile v1 certification."
    ...
    (out_dir / "pair-matrix.json").write_text(
        json.dumps(
            {"pairs": [{"pair": "v1_certification_gate", "status": overall_status, "classification": overall_class, "reason": overall_reason}]},
            indent=2,
            sort_keys=True,
        )
        + "\n",
    ...
    (out_dir / "coverage-summary.json").write_text(json.dumps({"milestone": 16, "prerequisite_count": len(prereq_rows), "status": overall_status}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    (out_dir / "device-matrix.json").write_text(json.dumps({"note": "Derived from prerequisite milestones."}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (out_dir / "os-matrix.json").write_text(json.dumps({"note": "Derived from prerequisite milestones."}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (out_dir / "language-matrix.json").write_text(json.dumps({"note": "Derived from prerequisite milestones."}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (out_dir / "library-matrix.json").write_text(json.dumps({"note": "Derived from prerequisite milestones."}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    gaps = ["# Portability Gaps (Milestone 16)", ""]
    ...
    (out_dir / "summary.md").write_text(
        "\n".join(
            [
                "# Milestone 16: Universal Profile v1 Certification",
                "",
    ...
    (out_dir / "milestone.json").write_text(
        json.dumps(
            {
                "milestone": 16,
                "slug": "universal-profile-v1",
                "owner": "Astrocytech/Glyphser",
                "target_date": "2026-07-31",
                "dependencies": sorted(REQUIRED_MILESTONES.keys()),
                "result": overall_status,
                "classification": overall_class,
                "evidence_dir": "evidence/repro/milestone-16-universal-profile-v1/",
            },
            indent=2,
            sort_keys=True,
        )
