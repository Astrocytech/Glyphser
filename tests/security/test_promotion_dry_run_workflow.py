...
def test_promotion_dry_run_workflow_exists_and_runs_promotion_gate() -> None:
    ...
    assert "python tooling/security/canary_promotion_guard.py" in wf
    assert "python tooling/security/rolling_merkle_checkpoints.py" in wf
    ...
    assert "canary_promotion_guard.json" in wf
    assert "rolling_merkle_checkpoints.json" in wf
