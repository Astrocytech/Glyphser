...
def test_runtime_api_state_schema_gate_fails_when_collision_provenance_missing(monkeypatch, tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _write_policy(repo)
    state_dir = repo / "artifacts" / "generated" / "tmp" / "security"
    state_dir.mkdir(parents=True, exist_ok=True)
        ...
        json.dumps(
            {
                "quotas": {
                    ...
                    "replay_window_by_job": {},
                    "replay_window_job_tokens": {},
                    "idempotency_collisions": {"idem-key-1": 1},
                },
    ...
    assert runtime_api_state_schema_gate.main([]) == 1
    report = json.loads((repo / "evidence" / "security" / "runtime_api_state_schema_gate.json").read_text("utf-8"))
    assert "missing_collision_provenance" in report["findings"]
