...
def test_key_rotation_cadence_gate_passes_with_signed_recent_record(monkeypatch, tmp_path: Path) -> None:
    ...
    accept_until = rotated_at + timedelta(days=10)
    policy.write_text(
        json.dumps(
            {
                "maximum_rotation_age_days": 90,
                "maximum_rollover_window_days": 30,
        ...
        json.dumps(
            {
                ...
                "previous_key_accept_until_utc": accept_until.isoformat(),
            }
        )
        + "\n",
        encoding="utf-8",
        ...
        json.dumps(
            {
                ...
                "maximum_rollover_window_days": 7,
                "rotation_record_path": "governance/security/key_rotation_record.json",
                "require_signed_rotation_record": True,
            }
        )
...
def test_key_rotation_cadence_gate_fails_on_expired_or_oversized_rollover_window(
    ...
    (gov / "key_management_policy.json").write_text(
        json.dumps(
            {
                "maximum_rotation_age_days": 2000,
                "maximum_rollover_window_days": 3,
                "rotation_record_path": "governance/security/key_rotation_record.json",
                "require_signed_rotation_record": True,
            }
        )
    ...
    record = gov / "key_rotation_record.json"
    rotated_at = datetime.now(UTC) - timedelta(days=1)
    accept_until = rotated_at + timedelta(days=20)
    record.write_text(
        json.dumps(
            {
                "key_id": "kid",
