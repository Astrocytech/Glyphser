...
def test_security_workflow_permissions_policy_gate_fails_on_workflow_default_write(
    monkeypatch, tmp_path: Path
) -> None:
    repo = tmp_path / "repo"
    wf = repo / ".github" / "workflows"
    ...
    report = json.loads((ev / "security_workflow_permissions_policy_gate.json").read_text(encoding="utf-8"))
    assert report["status"] == "FAIL"
    prefix = "forbidden_workflow_default_write_permission:.github/workflows/ci.yml:contents:write"
    assert any(item.startswith(prefix) for item in report["findings"])
