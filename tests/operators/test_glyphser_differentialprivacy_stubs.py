...
def test_glyphser_differentialprivacy_apply_stub_error():
    data = _load("Glyphser.DifferentialPrivacy.Apply")
    vector = data["vectors"][0]
    expected = vector["expected"]["error"]
    stub = getattr(gen_ops, "Glyphser_DifferentialPrivacy_Apply")
