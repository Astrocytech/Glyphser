...
ARTIFACT_PATHS = [
    ...
    ROOT / "artifacts" / "inputs" / "conformance" / "suites" / "hello-core" / "vectors-manifest.json",
    ROOT / "artifacts" / "inputs" / "conformance" / "suites" / "hello-core" / "vectors.json",
]
...
def run_verify() -> str:
    proc = subprocess.run(
        [sys.executable, str(ROOT / "tooling" / "docs" / "verify_doc_artifacts.py")],
        cwd=str(ROOT),
        check=True,
        capture_output=True,
        text=True,
